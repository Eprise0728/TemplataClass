  ...
  methods: {
    ...
    try {
      ...
      if (quizData && quizData.quizId === this.quizId) {
        ...
        const quizzes = response.data.quizResList.filter((quiz) => quiz.id === this.quizId);
...
<template>
    <div>
      ...
      <div class="playerdata">
        <p>姓名:<input type="text" v-model="playerData.name" /></p>
        <p>手機:<input type="text" v-model="playerData.phone" /></p>
        <p>E-mail:<input type="text" v-model="playerData.email" style="margin-left: 2.5%" /></p>
        <p>年齡:<input type="text" v-model="playerData.age" /></p>
      </div>
        ...
        <div class="question" v-for="(question, index) in questions" :key="index">
          ...
          <p class="questionans" v-if="question.type === '單選題' && question.options && question.options.length > 0">
            <label v-for="(option, optionIndex) in question.options" :key="optionIndex">
              <input type="radio" :name="'question' + question.id" :value="option" v-model="question.selectedOption" /> {{ option }}
            </label>
          </p>
  
          <p class="questionans" v-else-if="question.type === '多選題' && question.options && question.options.length > 0">
            <label v-for="(option, optionIndex) in question.options" :key="optionIndex">
              <input type="checkbox" :name="'question' + question.id" :value="option" v-model="question.selectedOptions" /> {{ option }}
            </label>
          </p>
  
          <p v-else-if="question.type === '簡答題'">
            <textarea :name="'question' + question.id" v-model="question.answer" placeholder="請輸入答案"></textarea>
          </p>
        </div>
      </div>
